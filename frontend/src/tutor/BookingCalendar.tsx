...
export function BookingCalendar({ csrfToken }: { csrfToken: string }) {
  ...
  function select(booking: Booking) {
    ...
    setStart(localInputValue(booking.start_at));
  ...
  function replaceBooking(updated: Booking) {
    ...
    setStart(localInputValue(updated.start_at));
  }
  ...
  async function moveBooking() {
    if (!selected) return;
    const response = await fetch(`/api/tutor/bookings/${selected.id}/schedule`, { method: "PUT", headers: { "Content-Type": "application/json", "X-CSRF-Token": csrfToken }, body: JSON.stringify({ start_at: new Date(start).toISOString(), override_id: overrideId || null, warning_acknowledged: acknowledged }) });
    if (response.ok) replaceBooking(await response.json());
  }

  const chosenOverride = overrides.find((item) => item.id === overrideId);
  return <section aria-labelledby="booking-calendar-heading"><h2 id="booking-calendar-heading">Weekly Booking Calendar</h2>{bookings.length === 0 ? <p>No Bookings.</p> : bookings.map((booking) => <button key={booking.id} onClick={() => select(booking)}>{booking.student.display_name} — {new Date(booking.start_at).toLocaleString()}</button>)}{selected ? <section aria-labelledby="calendar-booking-heading"><h3 id="calendar-booking-heading">Selected Booking</h3><p>{selected.student.display_name}</p><p>Funding: {selected.funding_kind}</p><label htmlFor="calendar-meeting-details">Meeting Details</label><textarea id="calendar-meeting-details" value={details} onChange={(event) => setDetails(event.target.value)} /><button onClick={saveDetails}>Save Meeting Details</button><label htmlFor="calendar-booking-start">Move Booking</label><input id="calendar-booking-start" type="datetime-local" value={start} onChange={(event) => setStart(event.target.value)} /><label htmlFor="calendar-override">Tutor Override</label><select id="calendar-override" value={overrideId} onChange={(event) => { const value = event.target.value; setOverrideId(value); setAcknowledged(false); const chosen = overrides.find((item) => item.id === value); if (chosen) setStart(localInputValue(chosen.start_at)); }}><option value="">Normal Bookable Slot</option>{overrides.map((item) => <option key={item.id} value={item.id}>{item.warning}</option>)}</select>{chosenOverride ? <label><input type="checkbox" checked={acknowledged} onChange={(event) => setAcknowledged(event.target.checked)} />I acknowledge: {chosenOverride.warning}</label> : null}<button onClick={moveBooking} disabled={Boolean(chosenOverride) && !acknowledged}>Move Booking</button></section> : null}</section>;
}
