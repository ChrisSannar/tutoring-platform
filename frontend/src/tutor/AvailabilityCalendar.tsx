...
function BlockedRow({ blocked, csrfToken, onDelete }: { blocked: BlockedTime; csrfToken: string; onDelete: () => void }) {
  const [start, setStart] = useState(blocked.start_at.slice(0, 16));
  const [end, setEnd] = useState(blocked.end_at.slice(0, 16));
  const [reason, setReason] = useState(blocked.reason ?? "");
  async function save() {
    await fetch(`/api/tutor/blocked-times/${blocked.id}`, { method: "PUT", headers: mutationHeaders(csrfToken), body: JSON.stringify({ start_at: new Date(start).toISOString(), end_at: new Date(end).toISOString(), reason: reason || null }) });
  }
  async function remove() {
...
export function AvailabilityCalendar({ csrfToken }: { csrfToken: string }) {
  ...
  async function addBlocked(event: FormEvent) {
    event.preventDefault();
    const response = await fetch("/api/tutor/blocked-times", { method: "POST", headers: mutationHeaders(csrfToken), body: JSON.stringify({ start_at: new Date(blockedStart).toISOString(), end_at: new Date(blockedEnd).toISOString(), reason: reason || null }) });
    if (response.ok) {
      const created = await response.json();
  ...
  async function addOverride(event: FormEvent) {
    event.preventDefault();
    const response = await fetch("/api/tutor/overrides", { method: "POST", headers: mutationHeaders(csrfToken), body: JSON.stringify({ start_at: new Date(overrideStart).toISOString(), warning }) });
    if (response.ok) {
      const created = await response.json();
